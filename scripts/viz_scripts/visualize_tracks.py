...
segtype_3d = 'DeepOCSORT_s5'
segtype_masa = 'masa_s5_Detic_thr0_1'

loader_2d = DevaLoader(f'out/{pid}/{vid}/segmaps/{segtype_2d}/')
loader_3d = DevaLoader(f'out/{pid}/{vid}/segmaps/{segtype_3d}/')
loader_masa = DevaLoader(f'out/{pid}/{vid}/segmaps/{segtype_masa}/')
...
visor_frames = set([x.split('.')[0] for x in os.listdir(observations.dir_visor)])
from eval_deva import init_visor_class_name2id
visor_class_name2id = init_visor_class_name2id(vid)

class_name = None
# class_name = "chopping board"
...
frames = sorted(loader_2d.frame2id)[::10]
...
if save_fig:
    utils.write_mp4(vid+'_tracks_DeepOCSORT'+(class_name if class_name else ''), ims, fps=5)
